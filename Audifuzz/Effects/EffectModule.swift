...
/// One adjustable knob on an effect. The UI turns each one into a dial.
struct EffectParameter: Identifiable {
    ...
    let unit: String
    let display: ((Float) -> String)?   // optional custom readout for the dial
    var value: Float

    init(id: String, name: String, range: ClosedRange<Float>, value: Float,
         unit: String = "", display: ((Float) -> String)? = nil) {
        self.id = id
        self.name = name
        self.range = range
